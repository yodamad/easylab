...
// OVHProvider implements the Provider interface for OVH
...
// GetRequiredEnvVars returns the list of environment variable names required by OVH
func (p *OVHProvider) GetRequiredEnvVars() []string {
	return []string{
		"OVH_APPLICATION_KEY",
...
// GetPulumiConfigPrefix returns the Pulumi config prefix for OVH
func (p *OVHProvider) GetPulumiConfigPrefix() string {
	return "ovh:"
}

// CreateInfrastructure creates OVH-specific infrastructure
func (p *OVHProvider) CreateInfrastructure(ctx *pulumi.Context, config providers.ProviderConfig) (*providers.InfrastructureResult, error) {
	ovhConfig, ok := config.(*OVHConfig)
	if !ok {
		return nil, fmt.Errorf("invalid config type for OVH provider: expected *OVHConfig")
	}

	serviceName := ovhConfig.ServiceName
	...
	kubeCluster, err := initManagedKubernetesCluster(ctx, serviceName, privateNetwork, subnet, gateway)
	if err != nil {
		return nil, fmt.Errorf("failed to create Kubernetes cluster: %w", err)
	}

	nodePools, err := initNodePools(ctx, serviceName, kubeCluster)
	if err != nil {
		return nil, fmt.Errorf("failed to create node pools: %w", err)
	}

	// Convert node pools to []pulumi.Resource
	nodePoolResources := make([]pulumi.Resource, len(nodePools))
	for i, np := range nodePools {
		nodePoolResources[i] = np
	}

	return &providers.InfrastructureResult{
		KubeCluster: kubeCluster,
		NodePools:   nodePoolResources,
